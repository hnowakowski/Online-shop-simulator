...
Clothing::Clothing(std::string             id,
                   std::string             name,
                   ...
                   uint32_t                size)
    : Product(id, name, description, image, rating, quantity, price), color(color), clothing_type(clothing_type),
      size(size)
{
}
...
Clothing::Clothing(const nlohmann::json& json) { this->Clothing::fromJSON(json); }
