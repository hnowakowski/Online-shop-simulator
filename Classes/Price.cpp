...
Price Price::operator+(const Price& price)
{
    uint32_t newmainprice = this->mainunit + price.mainunit + ((this->subunit + price.subunit) / 100);
...
Price Price::operator-(const Price& price)
{
    int newmainprice = this->mainunit - price.mainunit - (abs((int) this->subunit - (int) price.subunit) / 100);
